...
h_x, h_y = 0.212 * np.array([1.0, 1.0, -1.0, -1.0]), 0.14 * np.array([-1.0, 1.0, 1.0, -1.0])
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            vehicle.set_target_velocity(np.array([0.3, 0.3, 0.5]),"local")
            state = vehicle.hex_controller.get_state()
            print("vehicle_speed:", state["vehicle_speed"])
            # print("motor_velocity:", state["motor_velocity"])
            time.sleep(0.05)
